...
public final class QrCode {
    ...
    private JSONObject newJSONObject() {
        ...
        final var json = jsonTemplate.formatted(PFI, ARRANJO_PAGAMENTO, dadosPix.chavePIX().chave(), dadosPix.descricao(), MCC, COD_MOEDA, COD_CAMPO_VALOR, dadosPix.valorStr(), COD_PAIS, dadosPix.chavePIX().nome(), dadosPix.cidadeRemetente(), idTransacao);
    ...
    private String encodeValue(final String key, final Object val) {
        ...
        if (val instanceof JSONObject jsonObjValue) return generateInternal(jsonObjValue);
    ...
    public byte[] saveAndGetBytes(final Path imagePath) {
        ...
        try {
            if (code == null || code.isBlank()) generate();
