package com.marcelocbasilio.model.qrcode;
...
public record DadosEnvio(Chave chavePIX, BigDecimal valor, String cidadeRemetente, String descricao) {
    ...
    public DadosEnvio {
        ...
        if (chavePIX.nome().length() > TAMANHO_NOME) {
            final var msg = "Nome do destinatário não pode ter mais que 25 caracteres. '%s' tem %d caracteres.".formatted(nomeDestinatario, nomeDestinatario.length());
        ...
        if (chaveDestinatario.length() > TAMANHO_CHAVE) {
            final var msg = "Chave PIX do destinatário não pode ter mais que 77 caracteres. '%s' tem %d caracteres.".formatted(chaveDestinatario, chaveDestinatario.length());
            throw new IllegalArgumentException(msg);
        }
        ...
        if (cidadeRemetente.length() > TAMANHO_CIDADE) {
            final var msg = "Cidade do remetente não pode ter mais que 15 caracteres. '%s' tem %d caracteres.".formatted(cidadeRemetente, cidadeRemetente.length());
            throw new IllegalArgumentException(msg);
        }
        ...
        if (descricao.length() > TAMANHO_DESCRICAO) {
            final var msg = "Descrição não pode ter mais que 72 caracteres. '%s' tem %d caracteres.".formatted(descricao, descricao.length());
            throw new IllegalArgumentException(msg);
        }
        ...
        if (valorStr.length() > TAMANHO_VALOR) {
            final var msg = "Valor não pode ter mais que 13 caracteres. '%s' tem %d caracteres.".formatted(valorStr, valorStr.length());
            throw new IllegalArgumentException(msg);
        }
    }
